...
function Settings() {
  ...
  return (
    <Page width="narrow" className="space-y-8">
      ...
      <Section title="Actions" description="Configure app launches, websites, folders, commands and desktop controls per dock."><Group>{ACTION_DEFINITIONS.map((def) => { const Icon = ACTION_ICONS[def.type]; const ok = isSupported(def.type); return <Row key={def.type} icon={<Icon className="text-muted-foreground h-[1.15rem] w-[1.15rem]" />} title={def.label} subtitle={ok ? def.description : "Not supported on this computer"} trailing={<span className="text-[0.75rem]">{ok ? "Available" : "Unsupported"}</span>} className={ok ? "" : "opacity-55"} />; })}</Group></Section>
