...
export function DockSetup({ onFinish, onExit }: { onFinish?: (() => void) | undefined; onExit?: (() => void) | undefined }) {
  ...
  // Dock setup is already outside the normal onboarded/polling flow. Keep this
  // screen self-contained instead of mutating a global window flag during render.
  // The old global flag could leave the WebView in a bad interaction state when
  // the setup screen transitioned from hardware detection to the name step.
  ...
  useEffect(() => {
    ...
    const read = async () => {
      ...
      try {
        ...
        // Keep detection local until the user saves the dock. This prevents the
        // provider from starting activation while the name screen is interactive.
  ...
  return (
    <div className="fixed inset-0 z-[9999] isolate overflow-auto bg-background pointer-events-auto">
      <div className="mx-auto w-full max-w-xl px-4 pt-6 pb-10 sm:px-6 lg:pt-10">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
          <button type="button" onClick={goBack} className="press hover:bg-accent -ml-2 flex h-10 w-10 items-center justify-center rounded-full" aria-label="Back">
...
function StepName({ inputRef, name, setName, onContinue }: { inputRef: React.RefObject<HTMLInputElement>; name: string; setName: (v: string) => void; onContinue: () => void }) {
  ...
  return (
    <div>
      ...
      <Button size="lg" className="mt-8 h-12 w-full rounded-[var(--radius-xl)] text-[0.9375rem]" disabled={!canContinue} onClick={onContinue}>Continue</Button>
    </div>
  );
}
