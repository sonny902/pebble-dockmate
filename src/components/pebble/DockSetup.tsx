...
export function DockSetup({ onFinish, onExit }: { onFinish?: (() => void) | undefined; onExit?: (() => void) | undefined }) {
  ...
  const { saveDock, getDock, device, docks, placeOnDock } = usePebble();
  ...
  useEffect(() => {
    ...
    if (!input) return;
    const frame = window.requestAnimationFrame(() => { input.focus({ preventScroll: true }); input.select(); });
  ...
  useEffect(() => {
    ...
    if (device.dock != null) {
      ...
      if (first || found == null) {
        acceptDock(device.dock, device.connected, device.charging);
  ...
  }, [step, device.connected, device.charging, device.dock, first, getDock, placeOnDock]);
...
function StepName({ inputRef, name, setName, onContinue }: { inputRef: RefObject<HTMLInputElement | null>; name: string; setName: (v: string) => void; onContinue: () => void }) { const canContinue = name.trim().length > 0; return <div><Title title="What would you like to call this dock?" subtitle="Choose something you’ll recognise instantly." /><input ref={inputRef} autoFocus value={name} onChange={(e) => setName(e.currentTarget.value)} onKeyDown={(e) => { if (e.key === "Enter" && canContinue) onContinue(); }} placeholder="Desk Dock" aria-label="Dock name" className="bg-elevated border-hairline focus:ring-ring/40 h-14 w-full rounded-[var(--radius-xl)] border px-5 text-center text-lg font-medium shadow-[var(--shadow-1)] outline-none transition focus:ring-2" /><div className="mt-5 flex flex-wrap justify-center gap-2">{DOCK_NAME_SUGGESTIONS.map((s) => <button key={s} type="button" onClick={() => setName(s === "Desk" ? "Desk Dock" : s)} className="press border-hairline bg-elevated hover:bg-accent rounded-full border px-3.5 py-2 text-[0.8125rem] font-medium">{s}</button>)}</div><Button size="lg" className="mt-8 h-12 w-full rounded-[var(--radius-xl)] text-[0.9375rem]" disabled={!canContinue} onClick={onContinue}>Continue</Button></div>; }
