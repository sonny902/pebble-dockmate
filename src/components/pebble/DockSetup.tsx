...
export function DockSetup({ onFinish, onExit }: { onFinish?: (() => void) | undefined; onExit?: (() => void) | undefined }) {
  ...
  // Tell the provider to suspend its background status polling immediately.
  // Doing this during render matters because the first native dock-status call
  // can otherwise race the provider's 3-second polling loop on Tauri.
  if (typeof window !== "undefined") {
    (window as typeof window & { __PEBBLE_DOCK_SETUP__?: boolean }).__PEBBLE_DOCK_SETUP__ = true;
  }

  useEffect(() => () => {
    (window as typeof window & { __PEBBLE_DOCK_SETUP__?: boolean }).__PEBBLE_DOCK_SETUP__ = false;
  }, []);
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => window.requestAnimationFrame(focus));
  ...
  useEffect(() => {
    ...
    const read = async () => {
      ...
      try {
        ...
        // A remembered dock ID is not a placement event. Only a dock ID that
        // changes from the value present when this setup screen opened counts.
        if (currentDock == null || currentDock === baselineDock.current) return;

        // Keep setup detection local. Do not update global device state here:
        // that can trigger the provider's activation/polling machinery while
        // React is transitioning into the interactive name screen.
  ...
  const handleSave = () => { if (dockId == null || !name.trim()) return; saveDock({ id: dockId, name: name.trim(), actions }); toast.success(`${name.trim()} saved`, { description: `${actions.length} action${actions.length === 1 ? "" : "s"} configured` }); setStep("done"); };
  const progressIndex = Math.max(0, PROGRESS.indexOf(step === "duplicate" ? "waiting" : step));

  return <div className="relative z-50 pointer-events-auto mx-auto w-full max-w-xl px-4 pt-6 pb-10 sm:px-6 lg:pt-10">
    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3"><button type="button" onClick={goBack} className="press hover:bg-accent -ml-2 flex h-10 w-10 items-center justify-center rounded-full pointer-events-auto" aria-label="Back"><ArrowLeft className="h-5 w-5" /></button><div className="flex justify-center gap-1.5">{PROGRESS.map((s, i) => <span key={s} className={cn("h-1 rounded-full transition-all duration-500", i <= progressIndex ? "bg-primary w-7" : "bg-muted w-4")} />)}</div><span className="w-10" /></div>
    <div key={step} className="animate-rise relative z-50 pointer-events-auto mt-8">
...
function StepName({ inputRef, name, setName, onContinue }: { inputRef: React.RefObject<HTMLInputElement>; name: string; setName: (v: string) => void; onContinue: () => void }) { const canContinue = name.trim().length > 0; return <div className="relative z-[60] pointer-events-auto"><Title title="What would you like to call this dock?" subtitle="Choose something you’ll recognise instantly." /><input ref={inputRef} autoFocus value={name} onChange={(e) => setName(e.currentTarget.value)} onInput={(e) => setName(e.currentTarget.value)} onKeyDown={(e) => { if (e.key === "Enter" && canContinue) onContinue(); }} placeholder="Desk Dock" aria-label="Dock name" className="pointer-events-auto relative z-[60] bg-elevated border-hairline focus:ring-ring/40 h-14 w-full rounded-[var(--radius-xl)] border px-5 text-center text-lg font-medium shadow-[var(--shadow-1)] outline-none transition focus:ring-2" /><div className="mt-5 flex flex-wrap justify-center gap-2">{DOCK_NAME_SUGGESTIONS.map((s) => <button key={s} type="button" onClick={() => setName(s === "Desk" ? "Desk Dock" : s)} className="pointer-events-auto relative z-[60] press border-hairline bg-elevated hover:bg-accent rounded-full border px-3.5 py-2 text-[0.8125rem] font-medium">{s}</button>)}</div><Button size="lg" className="pointer-events-auto relative z-[60] mt-8 h-12 w-full rounded-[var(--radius-xl)] text-[0.9375rem]" disabled={!canContinue} onClick={onContinue}>Continue</Button></div>; }
