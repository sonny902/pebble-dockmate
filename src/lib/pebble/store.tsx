...
export function PebbleProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { try { const raw = window.localStorage.getItem(STORAGE_KEY); if (raw) { const saved = JSON.parse(raw) as PersistedState; setDocks(saved.docks ?? []); setOnboarded(Boolean(saved.onboarded)); if (saved.pebbleIdentifier) setDevice((p) => ({ ...p, id: saved.pebbleId ?? null, name: saved.pebbleId != null ? `Pebble ${saved.pebbleId}` : "Pebble", identifier: saved.pebbleIdentifier })); } } catch {} setHydrated(true); return () => { timers.current.forEach(clearTimeout); if (polling.current) clearInterval(polling.current); }; }, []);
  useEffect(() => { if (!hydrated) return; try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ pebbleId: device.id, pebbleIdentifier: device.identifier, onboarded, dockId: device.dock, docks } satisfies PersistedState)); } catch {} }, [hydrated, device.id, device.identifier, device.dock, onboarded, docks]);
  ...
  const refreshStatus = useCallback(async () => { if (!device.identifier) return; try { applyDeviceState(await hardware.getStatus()); } catch { setDevice((p) => ({ ...p, connected: false, dock: null, charging: false })); } }, [applyDeviceState, device.identifier]);

  useEffect(() => { if (!hydrated || !device.identifier) return; void (async () => { try { const next = await hardware.reconnectPebble(device.identifier); previousDock.current = next.dock; setDevice(next); if (next.dock != null) { const dock = docks.find((d) => d.id === next.dock); if (dock?.enabled) runSequence(dock); else setPhase("detected"); } } catch {} })(); polling.current = setInterval(() => void refreshStatus(), 1500); return () => { if (polling.current) clearInterval(polling.current); polling.current = null; }; }, [hydrated, device.identifier]);
