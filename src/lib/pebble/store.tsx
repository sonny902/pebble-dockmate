...
export function PebbleProvider({ children }: { children: ReactNode }) {
  ...
  const timers = useRef<ReturnType<typeof setTimeout>[]>([]), polling = useRef<ReturnType<typeof setInterval> | null>(null), previousDock = useRef<number | null>(null), skipReconnect = useRef(false), pollInFlight = useRef(false);
